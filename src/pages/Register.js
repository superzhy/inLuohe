...
export default class Register extends Component {
    ...
    _inputChange() {
        ...
        //     pwd = this.state.pwd;
        const {userName,pwd,ConfirmPwd,nickname,VerificationCode} = this.state;

        console.log('userName' + userName);
        console.log(pwd);
        if (userName && pwd&&ConfirmPwd&&nickname) {
            this.setState({
                inputReady: true
            })
        } else {
    ...
    _submit() {
        ...
        if(pwd.length<6){
            ...
            fetchRequest('user/register','POST',params).then((res)=>{
                console.log(res);
    ...
    fetchCode(userName){
        ...
        fetchRequest('user/register/code','POST',params).then((res)=>{
            console.log(res);
            if(res.code===1){
                this.refs.toast.show(res.message,3000);
            }else if(res.code===-1){
            ...
            <View style={styles.container}>
                <ScrollView style={{flex: 1, paddingHorizontal: 15, position: 'relative'}}>
                    ...
                    <View style={styles.inputBox}>
                        ...
                        <InputGroup
                            placeholder='请输入昵称' icon={require('../images/icon/user.png')} style={{marginTop: 32}}
                            iconStyle={{width: 16, height: 17}}
                            getValue={(text) => {
                                this.setState({
                                    nickname: text
                                }, () => {
